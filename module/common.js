...
exports.loadTemplateHeader = function (req, res, arr) {
    ...
    // console.log(req.session);
    // console.log(req.session.state_abbr);
    ...
    async.series({
        ...
        getDistributors: function (cb) {
            // getting product all categories
            q.all([user.Distributors(req, res, arr.config.mysql, q)]).then(function (results) {
        ...
        if (err) {
            ...
            // console.log('arr.loged.consultant_fee ', arr.loged.consultant_fee);
            // arr.distributors = results.getDistributors[0][0];
            /* ===================== end change by divyesh (added) ===================== */


            // console.log("load template headers ---- results is here : " + JSON.stringify(results.getMembership));
            // console.log(req.session);
            // console.log('loadTemplateHeader 1');
            ...
            // console.log('loadTemplateHeader 2');
            if (n < 1) {
            ...
            arr.category = results.getProductCategories[0][0]; //arr.category = results[0][0];
            ...
            if (uid != 0) {
                ...
                if (results.getUserInfo.user.state_abbr == '' && uid != 1) {
                    q.all(self.getStateAbbr('', config.mysql, q, req.session.state)).then(function (result1) {
                        ...
                        // req.session.state_abbr = result1[0][0].sc;
                        arr.loged.state_abbr = req.session.state_abbr = result1[0][0].sc;
                    }).fail(function (err) {
                        console.log(err.stack);
                        throw err;
            ...
            // console.log('getting stax detials : ' + JSON.stringify(results.getStaxDetails));
            // console.log('loadTemplateHeader 3');
            ...
            // console.log('loadTemplateHeader 4');
            ...
            if (results.getMembership[0][0].length > 0) {
                ...
                // console.log('loadTemplateHeader 5');
                // req.session.stax = results.getStaxDetails[0][0][0].stax;
                // req.session.stax_per = results.getStaxDetails[0][0][0].stax_per;
                // $arr.loged.stax = results.getStaxDetails[0][0][0].stax;
                // $arr.loged.stax_per = results.getStaxDetails[0][0][0].stax_per;
