...
<div class="container row sellp1">
    ...
    {if $mybids|count <=  0}
          <div class="row mblue_box1 tno_bids"> {$phrase.no_bids_found} </div>
    {else}
      {foreach $mybids as $key => $val}
       <div class="row sellp border-btm">
            ...
            <div class="col-md-9">
             ...
             <div class="gray_bbox">
             {$phrase.quantity} : {if $val.qty == ''} 1 {else if $val.qty != ''} {$val.qty} {/if}
             </div>
             <div class="gray_bbox">
             {$phrase.price}:<span class="curr_symbol">{$currency} </span> <span id="price{$val.proposed_amount - $val.refund_amount}" class="format">{$val.proposed_amount - $val.refund_amount}</span> {if $val.awarded}|  {$phrase.won_date}: {$val.adate} {/if}
             </div>
             </div>        
            <div class="clearfix">
                    {if $val.buynow == 1}
                    <button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="window.location='{$config.url}/dashboard/invoice/{$val.b_id}'">{$phrase.view_invoice}</button>
                    {if $val.release == 0}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="doAction('release_buy',{$val.b_id})">{$phrase.release_fund}</button>{/if}
                    {if $val.release == 0}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="doAction('cancel_buy',{$val.b_id})">{$phrase.cancel_fund}</button>{/if}
                    {if $val.release == 2}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;">{$phrase.fund_cancelled}</button>{/if}
                    {if $val.release == 1}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;">{$phrase.fund_released}</button>{/if}         
                    {if $val.is_digital == 1}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="window.location.href='{$config.url}/dashboard/download/{$val.b_id}'">Downloaded File</button>{/if}               
                    {if $val.shipping_info != ''}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="viewShippingTracking({$val.cart_id},{$val.b_id})">{$phrase.view_shipping_tracking}</button>{/if} 
                    {if $val.feedback == 0 and $val.release > 0}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="doAction('feedback_buy',{$val.b_id})">{$phrase.leave_a_feedback}</button>{/if}       
                    {if $val.feedback == 1 and $val.release > 0}<button type="button" class="btn btn-danger bttns" style="margin-top:5px;" onclick="feedbackReview({$val.b_id})">{$phrase.feedback_detail}</button>{/if}
    ...
    <div class="dpad_zero col-md-2 col-xs-12 pull-right">
        <div class="r_btns"><a class="btn btn-danger" href="{$config.url}/search">{$phrase.buy_an_item}</a></div>
