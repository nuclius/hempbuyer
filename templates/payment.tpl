        ...
        <div class="wrapper">
          <div class="auctions_black_authorize_top_bg">
            ...
            <div class="auctions_black_authorize_top_mid">
              <h2>{$phrase.authorize_authentication}</h2>
            </div>
            ...
            <div class="white_theme_tot_aut">
              <form method="POST" name="authorize_form" action="/package/pay" data-parsley-validate >
                ...
                <div class="card_detail">
                  <label>{$phrase.pay_type}:</label>
                  <select type="payment" name="payment">
                    <option value="paypal">{$phrase.paypal}</option>
                    <option value="authorize">{$phrase.authorize}</option>
                    <option value="stripe">{$phrase.stripe}</option>
                  </select>
                </div>
                <div id="cardPayments" style="display:none;">
                  <div class="card_detail">
                    <label>{$phrase.card_type}:</label>
                    <div class="select_card_type">
                      <div class="top_select">
                        <select name="creditCardType" class="select" title="select_card_type">
                          <option value="Visa" selected="">{$phrase.visa}</option>
                          <option value="MasterCard">{$phrase.mastercard}</option>
                          <option value="Discover">{$phrase.discover}</option>
                          <option value="Amex">{$phrase.american_express}</option>
                        </select>
                      </div>
                    </div>
                  </div>
                  <div class="card_detail">
                    <label>{$phrase.card_number}:</label>
                    <div class="select_card_type">
                      <input type="text" maxlength="19" name="creditCardNumber" title="Card Number" value="" data-parsley-group="block1" required>
                      <span>{$phrase.eg}: 654344545465455</span> <span style="float:left;clear:both;width:100%;font:bold 11px Arial, Helvetica, sans-serif;color:#f00;float:left;width:100%;clear:both;"> </span> </div>
                  </div>
                  <div class="card_detail">
                    <label>{$phrase.expiration_date}:</label>
                    <div class="month">
                    ...
                    <div class="year">
                      ...
                      <span>{$phrase.ex}: 2013</span> </div>
                  </div>
                  <div class="card_detail">
                    <label>{$phrase.cvv}:</label>
                    <div class="select_card_type">
                      <input type="text" maxlength="4" name="cvv2Number" title="CVV" value="" data-parsley-group="block1" required>
                      <span>{$phrase.eg}: 455</span> <span style="font:bold 11px Arial, Helvetica, sans-serif;color:#f00;float:left;clear:both;width:100%;"> </span> </div>
                  </div>
