                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        {if $val.parent_cat_name == 'Flower'}
                                                {$val.mk_price =($research_data.flower_price)/($research_data.flower_vol)}
                                                {$mk_vol = $research_data.flower_vol}
                                                <td>{$val.mk_price}/{$val.short_unit}</td>
                                        {/if}

                                        {if $val.parent_cat_name == 'Dry Concentrates'}
                                                {$val.mk_price =$research_data.dry_c_price / $research_data.dry_c_vol}
                                                {$mk_vol = $research_data.dry_c_vol}
                                                <td>{$val.mk_price}/{$val.short_unit}</td>
                                        {/if}

                                        {if $val.parent_cat_name == 'Oil Concentrates'}
                                            {$val.mk_price =$research_data.oil_c_price / $research_data.oil_c_vol}
                                            {$mk_vol = $research_data.oil_c_vol}
                                            <td>{$val.mk_price}/{$val.short_unit}</td>
                                        {/if}

                                        {if $val.parent_cat_name == 'Edibles'}
                                            {$val.mk_price =$research_data.edibles_price / $research_data.edibles_vol}
                                            {$mk_vol = $research_data.edibles_vol}
                                            <td>{$val.mk_price}/{$val.short_unit}</td>
                                        {/if}

                                        <td>{$val.mk_price - $val.p_unit_price}/{$val.short_unit}</td>
                                        <td>{$val.i_qty}</td>
                                        <td>{$mk_vol}</td>
                                        <td>{($val.i_qty / $mk_vol) * 100}%</td>
                                    </tr>
                                    {/foreach}
                                    <!-- <tr>
                                        <td>Sour Diesel</td>
