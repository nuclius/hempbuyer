        ...
        <div class="row">
            ...
            <div class="col-sm-6">
                <div class="form-inline wt-wrap">
                    <button class="btn btn-prev del-btn deleteTrashMessage">DELETE</button>
                </div>
            </div>
        </div>
        <div class="db-content">
            <div class="db-menu">
                <ul class="">
                    ...
                    <li><a href='/inbox/trash' class="active">Trash</a></li> 
                </ul>
            </div>
            <div class="db-right-con messages-con-wrap">
                <table class="table table-hover">
                    ...
                    <form action="{$config.url}/inbox/sent" method="post">
                        <tbody id="userDeleteMessageSection">
                            {include file="hmb_trash_message_template.tpl" nocache}
                        </tbody>
                    </form>
                </table>
            </div>
...
<div class="modal fade hb-modal" id="inbox-success" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <h4 class="text-center">MESSAGE SUCCESSFULLY DELETED</h4> 
                <div class="lsc-msg">
...
<script language="javascript" type="text/javascript">
    ...
    function searchMessage() {
        $('#userDeleteMessageSection').html('');
        $.ajax({
            type : 'POST',
            url  : '/inbox/userMessageInbox',
            data :  { page : $('#page').val(), action : "delete" },
            dataType : 'json',
            success  : function(data) {
                $('input[name="page"]').val(1);
                $('#userDeleteMessageSection').html(data.html);
            }
        });
    }
    ...
    function showHideDeleteAllButton() {
        $("body .checkOne:checked").length > 0 ? $('body .deleteTrashMessage').show() : $('body .deleteTrashMessage').hide();
    }

    var selectedMessageId = [];
    var selectedMessageRid = [];
    /*** Delete Inbox Message ***/
    $('body').on('click', '.deleteTrashMessage', function(e) {
        e.preventDefault();
