...
<div class="table-responsive">
    <table class="table table-bordered table-hover">
        <thead>
            <tr>
                <th width="30" style="min-width: 46px;">
                    <button type="button" name="release_multiple" id="release_multiple" class="btn btn-danger" onclick="milti_release_payment()" style="display: none;">Release</button>
                </th>
                <th width="30">No.</th>
                <th>InvoiceId</th>
        ...
        <tbody>
            ...
            <tr>
                <td>
                    {if $val.gateway == 'escrow'}
                        {if $val.release_status == 'pending'}
                            <input type="checkbox" class="release_checkbox" value="{$val.id}">
                        {/if}
                    {/if}
                </td>
                ...
                <td>
                    {if $val.gateway == 'escrow' && $val.status == 'paid'}
                        {if $val.release_status == 'pending'}
                            <button type="button" name="release" id="release" class="btn btn-primary mt-2px" onclick="release_payment('{$val.id}', {$val.release_amount}, '{$val.gateway}')">Release</button>
                        {else /}
                            <span>{$val.release_status}</span>
                        {/if}
                    {elseif $val.gateway != 'escrow' && $val.status == 'paid'}
                        {if $val.release_status == 'pending'}
                            {if $val.is_primary}
                                <button type="button" name="release" id="release" class="btn btn-primary mt-2px" onclick="release_payment('{$val.id}', {$val.release_amount}, '{$val.gateway}')">Release</button>
...
paginationData.url = '/admincp/withdrawal_request';
