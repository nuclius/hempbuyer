...
$(document).ready(function() {
    ...
    $(document).on("change", ".release_checkbox", function() {
        var total_checked = $(".release_checkbox:checked").length;
        console.log('total_checked', total_checked);
        if(total_checked) {
            $("#release_multiple").show();
        } else {
            $("#release_multiple").hide();
        }
    });
});
var is_report = 0;
...
function get_list() {
    ...
    $.ajax({
        type: 'POST',
        url: '/admincp/withdrawal_request',
        data: Obj,
        // dataType: 'json',
...
function milti_release_payment() {
    var ids = [];
    $(".release_checkbox:checked").each(function( index ) {
        ids.push($(this).val());
    });
    console.log('ids', ids);
    release_payment(ids, 0, 'escrow');
}
function release_payment(id, amount, gateway) {
    console.log('id', id);
    var text = '';
    if(amount) {
        text = "You want to release the amount of $" + formatMoney(amount);
    } else {
        text = "You want to release the amount";
    }
    swal({
        title: "Are you sure?",
        text: text,
