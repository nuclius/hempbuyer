...
<div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12" style="margin-top:2%;">
                        ...
                        <ol class="breadcrumb">
                            <li class="active">
                                <i class="icon flaticon-flower-with-leaves"></i> Escrow <i class="fa fa-angle-double-right"></i> <i class="fa fa-search"></i> List
                            </li>
                        </ol>
                    </div>
                </div>
...
<div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{$phrases.commission}</th>
                                        <th>{$phrases.shipping_info}</th>
                                 ...
                                 {foreach $payments as $key => $val}
                                  {$paymentscount =1}
                                    <tr>
                                        ...
                                        <td>{$val.uname}</td>
                                        <td>{$val.qty}</td>
                                        <td><span class="curr_symbol">{$currency}</span>{$val.amt}</td>
                                        <td>{if $val.paid == 1} {if $val.release == 1} Released <br />date:{$val.r_date} <br />
                                          ...
                                          <td><span class="curr_symbol">{$currency} </span>{$val.commission}</td>
                                        ...
                                        <td>
                                           ...
                                           {/if}
                                            {if $val.paid == 1}
                                            Payable Amount:<span class="curr_symbol">{$currency} </span>{$val.amt-$val.commission} <br />
                                          ...
                                          <br />
                                           <div style="display:inline">
                                            ...
                                            <span id="exclude{$val.buynow_id}">
                                              ...
                                              <input type="hidden" name="payer[{$val.buynow_id}][amount]" value="{$val.amt-$val.commission}" disabled/> Select To Pay
                                            </span>
                                           </div>
                                          {/if}
 ...
 <p style="padding-bottom:2%;">{$phrases.to_create_new_preapproval_key} <a href="#" onclick="doApprovalProcess(this)">{$phrases.click_here}</a></p>
