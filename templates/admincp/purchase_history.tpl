...
<div class="container-fluid">
    ...
    <div class="row m-top-20">
        <form enctype="multipart/form-data" class="admin_form" data-parsley-validate method="get" enctype="multipart/form-data" action="{$config.url}/{$loged.adminUrl}/purchase-history/search/{$market}">
            ...
            <table class="table table-bordered table-hover">
                <thead>
                    <tr>
                        ...
                        <th sort="seller_username" class="sortcol  {if $sort=='seller_username'}sort_{$direction}{/if} {if $sort!='seller_username'}sort{/if}">Seller Name</th>
                        ...
                        <th>Amount (<span class="curr_symbol">{$currency}</span>)</th>
                    </tr>
                </thead>
                <tbody>
                    {if $pagination>0}
                        ...
                        <tr>
                            ...
                            <td>
                                {if $val.seller_username == ''} - {/if}
                                {if $val.seller_username != ''} {$val.seller_username} {/if}
                            </td>
                            <td>
                                {if $val.username == ''} - {/if}
                                {if $val.username != ''} {$val.username} {/if}
                            </td>
                            <td>{$val.i_amount}</td>
                        </tr>
                        {/foreach}
        ...
        if(fquery != '') {
            ...
            for (let index = 0; index < params.length; index++) {
                ...
                if(params.length-1 == index) {
                    if(sorted == 0) {
                        sort=
                        href = base + '/{$loged.adminUrl}/purchase-history/search/?sort=' + sort_this + '&direction=asc&' + params.join('&');
                    } else {
                        href = base + '/{$loged.adminUrl}/purchase-history/search/?' + params.join('&');
                    }
                    window.location.href = href;
                }
            }
        } else {
            href = base + '/{$loged.adminUrl}/purchase-history/search/?sort=' + sort_this + '&direction=asc';
            window.location.href = href;
        }
