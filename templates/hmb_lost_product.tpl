         ...
         <div class="db-content">
            <div class="db-menu">
                <ul class="">
                    ...
                    <li><a href='/buyer/lostAuction' class="active">Auctions Lost</a></li>
            ...
            <div class="db-right-con">
                <form action="{$config.url}/buyer/lostAuction" method="post">
                    <div id="lostProductSection">
                        {include file="hmb_lost_product_template.tpl" nocache}
                    </div>
                </form>
            </div>
         </div>
...
<script language="javascript" type="text/javascript">
    ...
    function searchForm() {
        $('#lostProductSection').html('');
        $.ajax({
            type : 'POST',
            url  : '/buyerSearchLostAuctionPage',
            //data :  { page : $('#page').val(), status : "'open'" },
            data :  { page : $('#page').val(), status : "'lost'", sort_by : $('#sort_by').val() },
            dataType : 'json',
            success  : function(data) {
                $('#sort_by').val($('#sort_by').val());
                $('input[name="page"]').val(1);
                $('#lostProductSection').html(data.html);
            }
        });
    }
</script>