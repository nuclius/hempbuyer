...
<div class="col-md-12">
  <div class="selft26">{$phrase.leave_feedback﻿}</div>
</div>
</div>
<div class="row fbck-container">
  <form action="{$config.url}/dashboard/feedback/add" method="post" data-parsley-validate>
   <input type="hidden" value="{$projects.buynow_id}" name="bid" /> 
      ...
      <div class="col-md-9">
        ...
        <div class="seller_ftlbl">{$phrase.seller}:{$projects.name}</div>
        <div class="seller_ftlbl1">{$phrase.leave_feedback_for_seller}</div>
        <div class="seller_ftlbl">{$phrase.please_enter_your_overall_response_for_this_experience_positive_neutral_or_negative}</div>
