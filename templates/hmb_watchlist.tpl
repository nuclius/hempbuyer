         ...
         <div class="db-content">
            <div class="db-menu">
                <ul class="">
                    ...
                    <li><a href='/buyer/watchlist' class="active">Watchlist</a></li>
                </ul>
            </div>
            <div class="db-right-con">
                <form action="{$config.url}/buyer/watchlist" method="post">
                    <div id="watchlistSection">              
                        {include file="hmb_watchlist_template.tpl" nocache}
                    </div>
                </form>
            </div>
         </div>
...
<script language="javascript" type="text/javascript">
    ...
    function searchForm() {
        $('#watchlistSection').html('');
        $.ajax({
            type : 'POST',
            url  : '/buyerSearchWatchlist',
            //data :  { page : $('#page').val(), status : "'open'" },
            data :  { page : $('#page').val(), sort_by : $('#sort_by').val() },
            dataType : 'json',
            success  : function(data) {
                $('#sort_by').val($('#sort_by').val());
                $('input[name="page"]').val(1);
                $('#watchlistSection').html(data.html);
            }
        });
    }
</script>