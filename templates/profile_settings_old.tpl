  ...
  <div class="col-md-9 mobile_view" style="margin-bottom: 10px;">
    <form action="/profile_settings/update/" method="post" accept-charset="utf-8" enctype="multipart/form-data"  data-parsley-validate id="profile_form">
      ...
      <div class="row smsg_row">
        <div class="bg bg-success padding-20"> <i class="fa fa-warning"></i> {$phrase.your_profile_successfully_updated}. </div>
      </div>
      {/if}
      <div class="panel panel-default">
        <div class="panel-heading ct_headt">{$phrase.personal_profile}</div>
        <div class="panel-body" style="padding-bottom: 0;">
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-4">
              ...
              <button type="button" class="btn save_btn" onclick="triggerfile()">{$phrase.upload}</button>
            </div>
          ...
          <div class="row frm">
            ...
            <div class="col-xs-12 col-sm-6 col-md-6" >
              <input class="form-control" placeholder="Nick Name*"  name="nickname" value="{$users.nick_name}" data-parsley-required>
            </div>
          </div>
          <div class="row frmbx">
...
<span id="error-msg" style="color:#F00;" class="hide">{$phrase.invalid_number}</span>
          ...
          <div class="row">
            <div class="select">{$phrase.select_categories}</div>
          </div>
        ...
        <label class="option srch_olbl" id="cat{$val.id}">
          <input type="checkbox"  name="categories[]" pid="{$val.parent_id}" value="{$val.id}" />
           <span class="checkbox"></span>{$val.name}
            ...
            {foreach $subcategory as $key_subs => $val_subs}
            
            {if $val_sub.id == $val_subs.parent_id}
            <div class="ds col-xs-12 col-sm-12 col-md-12" style="margin-left: 60px;">
              <label class="option srch_olbl" id="cat{$val.id}"> {for $foo=1 to {$val_subs.depth}} -- {/for}
                <input class="forward_v2" type="checkbox" name="categories[]" pid="{$val_subs.parent_id}"  value="{$val_subs.id}" 


                >
                <span class="checkbox"></span>{$val_subs.name} </label>
            </div>
            {/if}
