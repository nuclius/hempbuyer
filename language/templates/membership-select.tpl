    ...
    <div class="col-lg-9" style="padding-right: 0;padding-left: 0;">
      <!--<div class="row"><div class="col-md-12">Total Bid count for this month - {$loged.monthly_bid_limit}</br>
        ...
        Total number of bids remains to post - {$loged.bid_limit - $loged.bidcountyear}</br>
         ...
         {if $membership_permission}<a href="{$config.url}/membership/membership_details"> {$pharse.view_details }</a>{/if}
      <h4 class="ds_brd" style="font-size: 24px;">{$pharse.setup_your_membership}</h4>
      <form name="setautobid" id="setautobid" action="/membership/package" method="post" data-parsley-validate required>
      <div class="col-md-4 padle  m-top-20 font-12" style="font-size: 18px;">{$pharse.select_your_role_:} 
      <select name="membership" class="form-control" required data-parsley-required-message="Please select role">

         <option value="">{$pharse.please_select_role}</option>
        {foreach $membership as $key => $val}
          <option value="{$val.id}">{$val.name}</option> 
        {/foreach}
      </select>  
      </div>
      <div class="col-md-12  m-top-20 font-12 padle" style="font-size: 18px;"><b>{$pharse.select_your_package_:} </b></div>
...
<section id="pricePlans">
    <ul id="plans">
      <li class="plan">
        <ul class="planContainer"  >
          <li class="title"><h2 class="bestPlanTitle" style="font-size: 20px;">{$pharse.package_name}</h2></li>
           <li class="price"><p class="bestPlanPrice">{$pharse.total_amount}</p></li>
          <li>
            <ul class="options" >
              <li></li>             
      ...
      <li class="plan adc_{$val.id} roles_{$val.membership_id} roles col-md-3 hide pad0" amount="{if $val.amount > 0} $ {$val.amount}{else if $val.amount <= 0} Free {/if}">
        <ul class="planContainer">
          ...
          <li class="price"><p >US $ {$val.amount}</p></li>
          <li>
        ...
        <div><button  type="button" class="btn padee1" name="plan" value="{$val.id}"  required amount="{if $val.amount > 0} $ {$val.amount}{else if $val.amount <= 0} Free {/if}"><!--<input type="radio" name="plan" value="{$val.id}"  required amount="{if $val.amount > 0} $ {$val.amount}{else if $val.amount <= 0} Free {/if}">-->{$pharse.select}</button></div>
      </li> 
      {/foreach}
    </ul> <!-- End ul#plans -->
  </section>
 ...
 <input type="hidden" name="plan"  />
       <div class="col-md-12  m-top-20 font-12 padle" style="font-size: 24px;">{$pharse.amount_:} <span class="amnted">{#pharse.please_select_package}</span></div>
       <div class="col-md-12 m-top-20 padle">
       <div class="col-md-2 padle">
       <button type="submit" value="Pay" id="btnsubmit" class="btn btn-success padee4" >{$pharse.pay}</button></div></div>
      </form>
      </div>
      <div class="col-lg-3">
