...
<div class="container-fluid"> 
  ...
  <div class="row">
    <div class="col-lg-12">
      <ol class="breadcrumb">
        <li class="active"> <i class="fa fa-sitemap"></i> {$pharse.feedback_field_edit} </li>
      </ol>
    </div>
  </div>
  <!-- /.row -->
  ...
  <div class="row">
    <div class="col-lg-12"> 
      <div class="alert alert-success"> <strong>{$pharse.saved!}</strong> {$pharse.was_successfully_changed} </div>
   ...
   <form method="POST" enctype="multipart/form-data" class="admin_form" data-parsley-validate name="add-product" id="feed-product" action="/admincp/feedback/update">   
     <input type="hidden" value="{$feedfields.id}" name="id" />  
      <div class="form-group">
        <label>{$pharse.feedback_title}</label>
     ...
     <div class="form-group">
        <label>{$pharse.feedback_order}</label>
