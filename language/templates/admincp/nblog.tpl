...
<div class="container-fluid"> 
  ...
  <div class="row">
    <div class="col-lg-12">
      <ol class="breadcrumb">
        <li class="active"> <i class="icon flaticon-flower-with-leaves"></i>{if $projects.id == 0 or $projects.id == ''} {$pharse.add} {else} {$pharse.edit} {/if} Blog <i class="fa fa-hand-o-right"></i> <i class="fa fa-edit"></i> <i class="fa fa-hand-o-right"></i> {$projects.title} </li>
      </ol>
    </div>
  </div>
  <!-- /.row -->
  <div class="row">
    <div class="col-lg-12"> {if $error == 'saved'}
      <div class="alert alert-success"> <strong>{$pharse.saved!}</strong> {$blogs.title} {$pharse.was_successfully_changed} </div>
      {elseif $error != ''}
   ...
   <form method="POST" enctype="multipart/form-data" class="admin_form" data-parsley-validate name="add-product" id="add-product" action="/admincp/blog/save">   
     <input type="hidden" value="{$blogs.id}" name="id" />  
      ...
      <div class="form-group">
        <label>{$pharse.upload_file}</label>
