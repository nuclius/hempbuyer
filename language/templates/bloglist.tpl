  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-sm-6 col-xs-12">
        <h1 class="page-title"> {$pharse.blog_post} </h1>
      </div>
...
<li>/</li>
<li class="current_item"><span><span>{$pharse.blog}</span></span></li>
        </ul>
              </div>
    </div>
  </div>
...
<div class="row">
  ...
  {$val.title}  </a></h3>
    ...
    <div class="post-meta-content">
      <span class="post-date updated"><time class="entry-date" datetime="{$val.date_add}">{$val.date_add}</time></span>{$pharse.posted_by} <span class="author vcard margin-less">{$val.first_name} {$val.last_name}      </span>
          </div>
  </div>
      <div class="post-summary">
    <p>{$val.description_short}..</p>
  </div>
   <a class="mybtn" href="{$config.url}/blog/{$val.id}">{$pharse.read_more}</a>
  ...
  <div class="col-md-3 col-sm-3">
    <div class="sidebar">
      ...
      <div id="recent-posts-2" class="widget widget_recent_entries">
        <h4 class="widget-title">{$pharse.recent_posts}</h4>
        <ul>
