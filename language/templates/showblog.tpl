...
<section id="content-main" class="clearfix">
  <div class="container"><div class="row">
    ...
    <h3 class="post-title entry-title">{$blog.title}</h3></article>
      ...
      <span class="post-date updated"><time class="entry-date" datetime="{$val.date_add}">{$blog.date_add}</time></span>{$pharse.posted_by} <span class="author vcard margin-less">{$blog.first_name} {$blog.last_name}      </span>
          </div>
