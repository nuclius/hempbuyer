...
app.get('/', function(req, res){
  ...
  if (req.session.userid>0) {
  	q.all([packages.getuserdetails(req, config.mysql, q, req.session.userid)]).then(function(result2) {
  		$arr['plan_id'] = result2[0][0][0].plan_id;
  		$arr['status'] = result2[0][0][0].status;
	});
  }
  q.all(packages.show(req,config.mysql,q)).then(function(results){ 
     $arr['packages'] = results[0];
  	 common.tplFile('packages_new.tpl');
     common.headerSet(1);
